...
export const insertNewUser = async (name, email, password, birthDate, code) => {
  ...
  const [newUser] = await pool.query(`INSERT INTO users (name, email, password, birth_date, code, create_code_time) 
                                      VALUES (?, ?, ?, ?, ?, ?)`,[name, email, password, birthDate, code, createCodeTime]);
  return successQuery(newUser);
};

export const updateVerifyCode = async (id, code) => {
  const createCodeTime = new Date();
  const [editCode] = await pool.query(`UPDATE users SET code = ? , create_code_time = ? WHERE id=?`,[code, createCodeTime, id]);
...
const successQuery = (user) => user.affectedRows > 0;
